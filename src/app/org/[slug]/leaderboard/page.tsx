  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-400 text-sm mt-0.5">Totaaloverzicht van deze groep</p>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden mb-6">
        {/* Header */}
        <div className="grid grid-cols-3 sm:grid-cols-4 px-4 py-3 border-b border-gray-100 text-xs font-medium text-gray-400 uppercase tracking-widest">
        ...
        {stats.length === 0 ? (
          ...
          <div
            key={s.id}
            className="grid grid-cols-3 sm:grid-cols-4 px-4 py-3.5 border-b border-gray-50 last:border-0 items-center"
          >
            <div className="col-span-2 flex items-center gap-3 min-w-0">
              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-semibold shrink-0
                ${i === 0 ? 'bg-amber-400 text-white' : i === 1 ? 'bg-gray-200 text-gray-500' : i === 2 ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-400'}`}>
                {i + 1}
              </span>
              <span className="font-medium text-gray-900 text-sm truncate">{s.username}</span>
            </div>
