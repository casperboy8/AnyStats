        ...
        <div className="space-y-2">
          {orgs.map(org => (
            <Link
              key={org.id}
              href={`/org/${org.slug}`}
              className="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-4 hover:border-gray-400 transition-colors group"
            >
              <div>
                <p className="text-sm font-medium text-gray-900">{org.name}</p>
                {org.description && (
                  <p className="text-xs text-gray-400 mt-0.5">{org.description}</p>
                )}
              </div>
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                org.role === 'owner' ? 'bg-amber-50 text-amber-700' :
                org.role === 'admin' ? 'bg-blue-50 text-blue-700' :
