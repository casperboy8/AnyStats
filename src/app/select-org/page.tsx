    ...
    <div className="min-h-screen bg-[#f9f9f8] flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-6">
          <h1 className="text-base font-semibold text-gray-900">Kies een groep</h1>
          <p className="text-sm text-gray-400 mt-1">Hallo {session.username}</p>
        </div>
        ...
        <div className="space-y-2">
          {orgs.map(org => (
            <Link
              key={org.id}
              href={`/org/${org.slug}`}
              className="flex items-center justify-between bg-white border border-gray-200 rounded-xl p-4 hover:border-gray-400 hover:shadow-sm transition-all group"
            >
